        ...
        <asp:ListView runat="server" ID="ListViewTopics" ItemType="Server.Models.Topic" DataKeyNames="ID" SelectMethod="ListViewTopics_GetData" InsertMethod="ListViewTopics_InsertItem"
            ...
            <InsertItemTemplate>
                <%if (Context.User.Identity.GetUserId() != null)
                    { %>
                <a href="#panelInsertTopic" id="buttonShowInsertPanel" class="btn btn-info pull-right" onclick="(function (ev) { $('#panelInsertTopic').show(); $('#buttonShowInsertPanel').hide(); }())">Insert new Topic</a>
                <%} %>
            ...
            <ItemTemplate>
                <div class="row topicHeader">
                    <h3>
                        <asp:HyperLink NavigateUrl='<%# "~/ViewTopic?id=" + Item.Id %>' runat="server" Text="<%#: Item.Title %>" />
                    </h3>
                </div>
